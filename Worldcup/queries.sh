...
echo "$($PSQL "SELECT DISTINCT teams.name FROM teams INNER JOIN games ON teams.team_id IN (games.winner_id, games.opponent_id) WHERE games.year = 2014 AND games.round = 'Eighth-Final'")"
...
echo "$($PSQL "SELECT DISTINCT games.year, teams.name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE games.round = 'Final'")"
